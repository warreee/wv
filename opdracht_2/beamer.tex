...
\note{granularity\\}
\note{Monolithisch = OS + app\\
applicationbased = os en apps gescheiden
component-based = at runtime component in app}
\end{frame}
...
\begin{frame}[label=sec-3-3]{Meetopstelling}
  ...
  \end{figure}
  \note{
    We willen het stroomverbruik van bepaalde methodes plotten. De
opstelling is als volgt: er wordt een weerstand in serie geplaatst met
de stroomtoevoer van de sensor node. De oscilloscoop zal de spanning
in het circuit meten. We laten de oscilloscoop metingen starten en
stoppen m.b.v. een output-pin op de sensor node. Deze pin zal
softwarematig aan en uit gezet worden door middel van een speciale
methode.

Meetmethode: meerdere keren het te meten stuk code oproepen. Pin zal
verscheidene keren aan en uit flippen en metingen triggeren.
  }
\end{frame}
