...
kan men abstractie maken van de low-level details van simulatie e.d., en in de plaats
in plaats daarvan een eenvoudige vuistregel gebruiken (een \textit{code
...
kan dus voordeliger uitkomen dan $x$ en $y$ bytes apart te verzenden. Let wel: $transfer$ is wel lineair in de zin dat als we de kost ervan plotten, we een lijn te zien krijgen. } 
...
\begin{itemize}
\item \textbf{opslag}: hoeveel kost het om $n$ bytes op te slaan in het RAM?\footnote{Men zou ook geneigd zijn om data op te slaan op het EEPROM. In dit onderzoek is dat echter achterwege gelaten omdat dat blijkbaar niet vaak gebruikt wordt en bovendien een hoge kost blijkt te hebben ten opzichte van opslag in het RAM.}
...
laten is het verbruik door de antenne, dit omdat we dit niet werkende kregen.
...
geenszins een probleem, aangezien onze metric dan nooit te snel gaat zeggen dat
