...
opslaan vol is.\footnote{\todo[inline]{Het is wel lineair, maar door de grote opstartkost kan het dus verschillen zodanig dat 2 keer apart verzenden meer kost dan 1 enkele keer}Merk ook op dat de kost van het verzenden van gegevens
...
het EEPROM? 
\todo[inline]{EEPROM, niet hier vermelden, wel zeggen dat we dit onderzocht hebben maar dat dit nadelen heeft en dat we het daarom niet verder onder de loop hebben genomen}
...
Om de juistheid van onze metric te controleren hebben we het scenario van de temperatuurmetingen bestudeerd. Dit scenario hebben we door onze tool laten berekenen om te kijken of het resultaat hiervan overeen kwam met het energieverbruik volgens de oscilosscoop. Wat we hier wel buiten beschouwing laten is het verbruik door de antenne, dit omdat we dit niet werkende kregen. 
\todo[inline]{Kunt ge dat beter verwoorden, "niet werkende krijgen"?}

In volgende tabel tonen we het temperatuurscenario waarbij het gemiddelde wordt berekend voor een bepaalde hoeveelheid metingen. De twee daarop volgende kolommen tonen respectievelijk het verbruik gemeten door de oscolisscoop en de hoeveelheid enrgie die geschat zou worden door onze tool.
...
\hline 
\cellcolor[gray]{0.8}Aantal bytes & \cellcolor[gray]{0.8} Oscolisscoop  & \cellcolor[gray]{0.8} Metric & \cellcolor[gray]{0.8} Overschatting \\ 
\hline 
...
De laatste kolom geeft de afwijking van onze metric weer. Ze toont dus een lichte overschatting van de hoeveelheid energie die verbruikt wordt. Dit is geenszins een probleem aangezien onze metric dan nooit te snel gaat zeggen dat een bepaalde berekening beter kan worden uitgevoerd op de mote.
